...
CONFIG = {
    ...
    "CHUNK_SEC":       5,
    "AUDIO_DEVICE":    1,       #  (USB 2.0 Device mikrofon)
...
energy_low = 0
def identify_speaker(wav_path: Path, threshold=0.7) ->  Optional[str]:
    # === Wav uzunluğu ve enerji kontrolü ===
    ...
    # === Ek karar: en iyi skor yeterli değilse veya fark azsa ===
...
def SYS_PROMPT() -> str:
    return (
        ...
        "Eğer kullanıcıdan gelen cümle anlamlı bir Türkçe ifadeyse ama bu robotun tanıdığı bir komut değilse: (örneğin: 'zıpla', 'uç', 'yüz', 'çay demle', 'elbise giy', 'kahve koy','yemek ye', 'dans et','duş al','namaz kıl','onu getir','dök','renk tespiti yap') gibi emir içeren kelimeler ise \n"
        "→ [{'komut': 'gecersiz', 'girdi': 'kullanıcının söylediği komut'}]\n\n"
        "Eğer kullanıcıdan geri dön gibi bir komut gelirse 180 derece sağa dön "
...
async def vad_task(q_raw: asyncio.Queue, q_stt: asyncio.Queue):
    ...
    while True:
        ...
        # 1) Önce araç aktif mi kontrol et
        if not client_sender.is_arac_aktif():
            logger.info("[VAD Task] Araç pasif - ses işlenmiyor")
            q_raw.task_done()
            continue

        # 2) Konuşma var mı kontrol et
    ...
    while True:
        ...
        async def handle_command(c):
            ...
            if cmd == "gecersiz":
                ...
                # Basit fiil sonlarını çekimli hale getir
                ...
                elif girdi.endswith("et"):  # 'et' ile biten kelimeler için özel durum
        ...
        async def send_and_speak(cmd, dur, ang):
            ...
            # TTS için küçük harf kullanabilirsin (Türkçe doğal dursun)
            wav = await openai_tts([cmd_obj])
            await play_wav(wav)

            # Arduino’ya gönderirken büyük harf kullan
